...
if CLIENT and not ZSDrawFix then
	-- localize draw functions because lua is a resource hog and can't navigate tables
	-- apologies to Garry for c&ping his gmod draw functions, don't send us DMCA takedown notices plox
		
	-- don't override it multiple times, because that would be retarded
	ZSDrawFix = true
	...
	-- CACHE EVERYTHING AND ANYTHING, MINOR FPS IMPROVEMENTS AHOY!
	local FontHeightCache = {}
	local function draw_GetFontHeight(font)
		if FontHeightCache[font] then
	...
	local CachedFontSizeW = {}
	local CachedFontSizeH = {}
	local function surface_GetTextSizeCached(text, font)
		if CachedFontSizeW[font] == nil then CachedFontSizeW[font] = {} end
		if CachedFontSizeH[font] == nil then CachedFontSizeH[font] = {} end
		
		if CachedFontSizeW[font][text] and CachedFontSizeH[font][text] then
			return CachedFontSizeW[font][text], CachedFontSizeH[font][text]
		end
		
		surface_SetFont(font)
		local w, h = surface_GetTextSize(text)
		CachedFontSizeW[font][text] = w
		CachedFontSizeH[font][text] = h
	...
	local function draw_SimpleText(text, font, x, y, colour, xalign, yalign)
		...
		if xalign == TEXT_ALIGN_CENTER then
			local w, h = surface_GetTextSizeCached( text, font )
			x = x - w / 2
		...
		return surface_GetTextSizeCached(text, font)
	end
	...
	local function draw_DrawText(text, font, x, y, colour, xalign )
		...
		for i=1, #text do
			...
			elseif ch == "\t" then
				...
				local tmpSizeX,tmpSizeY =  surface_GetTextSizeCached(curString, font)
	...
	local function team_GetColor(index)
		if TeamColorCache[index] then
			return table_Copy(TeamColorCache[index])
		end
		
		local TeamInfo = team_GetAllTeams()
		local color = COLOR_WHITE
		
		if not TeamInfo[index] then
			local defaultcolor = Color(255, 255, 100, 255)
			TeamColorCache[index] = defaultcolor
			return TeamColorCache[index]
		end
		
		TeamColorCache[index] = TeamInfo[index].Color
		
		return table_Copy(TeamColorCache[index])
	end
	
	local oldCreateFont = surface.CreateFont
	local function surface_CreateFont(name, fonttbl)
		fonttbl.extended = true
		oldCreateFont(name, fonttbl)
	end
	...
	draw.Text = draw_Text
	surface.GetTextSizeCached = surface_GetTextSizeCached
	team.GetColor = team_GetColor
	surface.CreateFont = surface_CreateFont
end
